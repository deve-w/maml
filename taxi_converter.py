#!/bin/python
# encoding: utf8
#to do: check if node moves since last reading (if not, don't need to calculate distances again).
###### Do you actually need to calculate the distance to every other node that has moved? -> not because when he moves he already calculates.
...
def verify_distance(line):
    ...
    for key in node_positions.keys():
        if key != line["id"]:
            ...
            distance = geodesic(item["position"],line["position"]).meters
...
def main():
    ...
    global clock
    args = conf_argparser()
    datatrace = pd.read_table(r'C:\Users\Dell\PycharmProjects\maml\data\romataxi\datatrace.txt', encoding='ISO-8859-1',
                              sep=',', header=None)
